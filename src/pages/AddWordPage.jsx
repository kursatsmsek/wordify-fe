    ...
    <div className="flex justify-center py-8 px-4">
      <div className="max-w-[960px] w-full">
        ...
        <div className="mb-8 animate-fade-up">
          <h1 className="text-slate-900 dark:text-slate-100 text-4xl font-black leading-tight tracking-tight mb-1">
            Add New Entry
          </h1>
          <p className="text-slate-500 dark:text-slate-400 text-sm">
            Manually add a word or import via JSON payload.
          </p>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          >
            <section className="bg-white dark:bg-slate-900 p-6 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-sm">
              ...
              <div className="space-y-5">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                  <Field label="English Word" required error={errors.english}>
                    <input
                      ...
                      value={form.english}
                      onChange={(e) => {
                        setForm((f) => ({ ...f, english: e.target.value }));
                        setErrors((er) => ({ ...er, english: "" }));
                      }}
                  ...
                  >
                    <input
                      ...
                      value={form.turkish}
                      onChange={(e) => {
                        setForm((f) => ({ ...f, turkish: e.target.value }));
                        setErrors((er) => ({ ...er, turkish: "" }));
                      }}
                ...
                <div className="space-y-3">
                  ...
                  {form.examples.map((ex, idx) => (
                    <div key={idx} className="flex gap-2 items-center">
                      <input
                        ...
                        value={ex}
                        onChange={(e) =>
                          handleExampleChange(idx, e.target.value)
                        }
                  ...
                  <button
                    onClick={addExample}
                    className="flex items-center gap-2 text-primary text-sm font-bold mt-1 hover:underline"
                  >
            ...
            <div className="flex justify-end gap-3">
              <button
                onClick={() => navigate("/vocabulary")}
                className="px-6 py-3 rounded-xl border border-slate-200 dark:border-slate-700 text-slate-600 dark:text-slate-400 font-bold hover:bg-slate-50 dark:hover:bg-slate-800 transition-colors text-sm"
              >
                Cancel
              </button>
              <button
              ...
              >
                <span className="material-symbols-outlined text-sm">
                  {saved ? "check_circle" : saving ? "refresh" : "save"}
                </span>
          ...
          >
            <section className="bg-white dark:bg-slate-900 p-6 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-sm">
              ...
              <p className="text-slate-500 dark:text-slate-400 text-xs mb-4">
                Paste a JSON payload to auto-fill the form.
              </p>
              <textarea
                value={jsonInput}
                onChange={(e) => {
              ...
              <button
                onClick={parseJSON}
                className="w-full mt-3 py-2 bg-primary/10 text-primary rounded-xl font-bold text-sm hover:bg-primary/20 transition-colors"
              >
                Parse JSON
              </button>
            </section>
            ...
            <section className="bg-primary/5 dark:bg-primary/10 p-6 rounded-2xl border border-primary/10 dark:border-primary/20">
              <h3 className="text-primary font-bold text-xs mb-3 uppercase tracking-wider">
                Quick Tips
              </h3>
              <ul className="text-slate-600 dark:text-slate-400 text-xs space-y-2.5">
                {[
                  "Use clear, concise sentences.",
                  "Levels help target your quizzes.",
                  "JSON import is best for bulk adding.",
                ].map((tip) => (
