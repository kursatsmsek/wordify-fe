...
export default function QuizPage() {
  ...
  const { quizSession, quizResults, settings, answerQuestion, skipQuestion } =
    useApp();
  ...
  const isTypingMode = settings.quizMode !== "MULTIPLE_CHOICE";
  ...
  useEffect(() => {
    if (!isTypingMode && currentWord && quizSession) {
      // 4 wrong options, 1 correct (total 5)
      const otherWords = quizSession.words.filter(
        (w) => w.id !== currentWord.id,
      );
      // If not enough words in session, fill from all words
      let pool = otherWords;
      if (pool.length < 4 && window.MOCK_WORDS) {
        // fallback: use global mock words if available
        pool = window.MOCK_WORDS.filter((w) => w.id !== currentWord.id);
      }
      const shuffled = [...pool].sort(() => Math.random() - 0.5).slice(0, 4);
      const options = [
        ...shuffled.map((w) =>
          quizDirection === "TR_TO_EN" ? w.english : w.turkish,
        ),
        correctAnswer,
      ].sort(() => Math.random() - 0.5);
      setMcOptions(options);
    }
  }, [currentWord?.id, isTypingMode, quizSession, quizDirection]);
          ...
          {isTypingMode && (
            <div className="w-full max-w-sm flex flex-col gap-4">
              <input
                ...
                className={`w-full px-6 py-4 rounded-xl text-center text-lg font-semibold transition-all outline-none border-2 ${
                  feedback === null
              ...
              {feedback && (
                <div
                  className={`flex items-center gap-2 justify-center text-sm font-semibold animate-fade-up ${feedback.correct ? "text-green-600 dark:text-green-400" : "text-red-600 dark:text-red-400"}`}
                >
              ...
              {mcOptions.map((option) => {
                ...
                return (
                  <button
                    ...
                    disabled={!!feedback}
                    className={`p-4 rounded-xl font-medium text-sm transition-all text-center border-2 ${style}`}
                  >
                    {option}
                  </button>
                );
